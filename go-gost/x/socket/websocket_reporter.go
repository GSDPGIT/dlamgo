...
func (w *WebSocketReporter) connect() error {
	...
	currentURL := buildWebSocketURL(w.addr, "/system-info", query)
	...
	headers := http.Header{}
	headers.Set("X-Node-Secret", w.secret)
...
func StartWebSocketReporterWithConfig(addr string, secret string, http int, tls int, socks int, version string) *WebSocketReporter {
	...
	fullURL := buildWebSocketURL(addr, "/system-info", query)
...
func buildWebSocketURL(addr string, path string, query url.Values) string {
	scheme := "ws"
	...
	return scheme + "://" + addr + path + "?" + query.Encode()
}
