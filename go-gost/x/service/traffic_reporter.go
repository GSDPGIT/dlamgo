...
func SetHTTPReportURL(addr string, secret string) {
	httpReportURL = buildHTTPReportURL(addr, "/flow/upload")
	configReportURL = buildHTTPReportURL(addr, "/flow/config")
...
func sendTrafficReport(ctx context.Context, reportItems TrafficReportItem) (bool, error) {
	...
	req.Header.Set("X-Node-Secret", httpReportSecret)
...
func buildHTTPReportURL(addr string, path string) string {
