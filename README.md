...
- `ios-app`：iOS 客户端工程
- `docker-compose-v4.yml` / `docker-compose-v6.yml`：历史 Compose 文件，**归档/兼容用途**
- `panel_install.sh` / `install.sh`：历史脚本，**归档/兼容用途**
...
- `springboot-backend`：旧 Java 后端，已归档
- `android-app`：Android 客户端，非默认部署路径
- `ios-app`：iOS 客户端，非默认部署路径
- `flux.ipa`：历史产物，非默认部署路径
...
仓库里仍保留 `springboot-backend`，但当前默认部署和本文档都不再使用它。
